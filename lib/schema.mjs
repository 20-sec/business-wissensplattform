...
export const revisionSchema = z.object({ id, createdAt: date, previousId: id.nullable(), reason: z.string().min(1), methodVersion: z.literal("1.0"), assessment: z.string().min(1), originalHash: z.string().regex(/^[a-f0-9]{64}$/).optional(), originalFilename: z.string().min(1).optional(), ideaExamples: z.array(z.object({ slogan: z.string().min(1), motif: z.string().min(1), mechanism: z.string().min(1), audience: z.string().min(1) }).strict()).optional(), keyFacts: z.array(z.object({ label: z.string().min(1).max(80), fact: z.string().min(1).max(500), implication: z.string().min(1).max(400), sourceIds: z.array(id).min(1) }).strict()).length(5).optional(), marketExamples: z.array(z.object({ title: z.string().min(1), author: z.string(), niche: z.string(), match: z.string(), language: z.string(), format: z.string(), implementation: z.string(), takeaway: z.string(), amazonUrl: url, caveat: z.string() }).strict()).optional(), compactSummary: z.array(z.object({ label: z.string().min(1).max(60), text: z.string().min(1).max(400) }).strict()).length(5).optional(), claims: z.array(claimSchema).min(1), sources: z.array(sourceSchema), personalRelevance: z.object({ rating: z.number().int().min(0).max(4).nullable(), rationale: z.string(), confirmedByMarkus: z.boolean() }).strict(), questions: z.array(z.string()), relatedVideoIds: z.array(id), reviewers: z.array(z.object({ role: z.enum(["Recherche", "Gegenprüfung", "Synthese"]), name: z.string().min(1), conclusion: z.string().min(1) }).strict()).min(2) }).strict().superRefine((r, ctx) => {
 ...
 if (!!r.originalHash !== !!r.originalFilename) ctx.addIssue({ code: "custom", message: "Revisionsoriginal benötigt Hash und Dateiname" });
});
export const videoSchema = z.object({ id, title: z.string().min(1), sourceUrl: url.nullable(), channel: z.string().nullable(), publishedAt: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable(), importedAt: date, categories: z.array(z.enum(categories)).min(1), sections: z.array(z.object({ category: z.enum(categories), section: z.string().min(1) }).strict()).min(1), tags: z.array(z.string().min(1)), summary: z.string(), originalHash: z.string().regex(/^[a-f0-9]{64}$/), originalFilename: z.string(), sourceAssessment: z.string().nullable(), topicIds: z.array(id), revisions: z.array(revisionSchema) }).strict();
...
export const catalogSchema = z.object({ schemaVersion: z.literal(1), videos: z.array(videoSchema), topics: z.array(topicSchema), reviewQueue: z.array(z.object({ videoId: id, triggerVideoId: id, reason: z.string().min(1), createdAt: date }).strict()) }).strict().superRefine((c, ctx) => {
 ...
 for (const v of c.videos) {
   ...
   if (v.sections.some(s => !v.categories.includes(s.category) || !taxonomy[s.category]?.includes(s.section)) || v.categories.some(c => !v.sections.some(s => s.category === c)) || new Set(v.sections.map(s => s.category + "/" + s.section)).size !== v.sections.length) ctx.addIssue({ code: "custom", message: "Jeder Oberbereich benötigt gültige, eindeutige Untersektionen" });
   const seen = new Set();
