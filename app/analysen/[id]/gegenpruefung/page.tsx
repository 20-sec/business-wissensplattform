...
export default async function Analysis({ params }: { params: Promise<{ id: string }> }) {
 ...
 return <main className="article"><a href={`/analysen/${video.id}`}>Zur Originalanalyse</a><p className="eyebrow" style={{ marginTop: 24 }}>UNSERE GEGENPRÜFUNG</p><h1>{video.title}</h1><p>{video.summary}</p><p className="muted">{video.categories.join(" · ")} · Importiert am {new Date(video.importedAt).toLocaleDateString("de-DE")}</p>{video.sourceUrl && <p><a href={video.sourceUrl} target="_blank" rel="noreferrer">Videoquelle öffnen</a></p>}
