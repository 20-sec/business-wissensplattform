...
test('Import schützt Originale, erzeugt Prüfimpulse und hängt Revisionen ausschließlich an', async () => {
 ...
 try {
  ...
  await writeFile(meta, JSON.stringify({ id: 'video-a', title: 'A', categories: ['KI & Automation'], tags: ['Agenten'], sourceAssessment: 'Ursprüngliches Rating: 8/10' }));
  ...
  await writeFile(meta, JSON.stringify({ id: 'video-b', title: 'B', categories: ['KI & Automation'], tags: ['Agenten'] }));
  success(run('import', html, meta));
